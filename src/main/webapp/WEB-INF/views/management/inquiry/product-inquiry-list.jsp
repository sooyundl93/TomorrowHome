...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %> 
      ...
      <div class="row">
        <div class="cell">
          ...
          <select id="search_option" name="search_option" class="drop-down">
            ...
            <option value="상품 코드" <c:if test="${ search_option eq '상품 코드' }">selected</c:if>>
              상품 코드
            </option>
      ...
      <div class="row advanced_search">
        <div class="cell">
          ...
          <select 
            ...
            <option value="상품 코드 오름차순" <c:if test="${ current_sort eq '상품 코드 오름차순' }">selected</c:if>>
              상품 코드 오름차순
            </option>

            <option value="상품 코드 내림차순" <c:if test="${ current_sort eq '상품 코드 내림차순' }">selected</c:if>>
              상품 코드 내림차순
            </option>
  ...
  <div class="contents" id="contents">
    ...
    <section class="table_top">
      <!-- ##### 인스턴스 레코드의 수 ##### -->
      <span class="caption">
        상품 문의 목록 <c:out value="${ numberOfRecords }" />건
      </span>
    ...
    <!-- ################################ 테이블 ################################ -->
    <section class="table product-inquiry-list">
      ...
      <c:forEach var="item" items="${ relationInstance }" varStatus="loopStatus">
        <div 
          ...
          <input type="hidden" class="url" value="product_inquiry_reply" />
          <div class="row">
            <span class="cell">
              <button 
                type="button" 
                class="accordion_button"
                onclick="openAccordion( 'product_inquiry_reply_list', '${ item.inquiry_no }' )">
                ${ item.inquiry_no }
              </button>
            </span>
            ...
            <span class="cell">
              <button 
                ...
                onclick="openAccordion('product_inquiry_reply_list', '${ item.inquiry_no }' )">
                <c:choose>
                  <c:when test="${ item.private_flag eq 'Y' }">
                    공개
                  </c:when>
                  <c:otherwise>
                    비공개
            ...
            <span class="cell">
              <button 
                type="button" 
                class="accordion_button numberOfReplies" id="numberOfRecords_${ item.inquiry_no }"
                onclick="openAccordion('product_inquiry_reply_list', '${ item.inquiry_no }' )">
          ...
          <div class="reply_to">
            ...
            <div class="button_container">
              <button type="button" class="brand-pink"
                onclick="uploadReply( 'product_inquiry_reply_upload', '${ item.inquiry_no }', '${ loopStatus.index }' )">
                답글 등록
              </button>
            </div>
          </div>
